...
contains
    ...
    subroutine print_help()
        print *, 'Usage: mpiexec/mpirun [-n/-np $NUM_PROC] program_name [-n SIZE] [-l LOOP_NUM] [-float|-double]'
    ...
    subroutine perform_float(dim, loop_num, ave_gflops, max_gflops, ave_time, min_time, my_rank, num_procs)
        ...
        do i = 1, loop_num
            if (my_rank == 0) then
                ...
                check_value = 0.0
                do k = 1, dim
                    check_value = check_value + a_float((check_indices(1)-1)*dim + k) * b_float((k-1)*dim + check_indices(2))
                end do
            end if
            ...
            ! 主进程进行校验
            if (my_rank == 0) then
                result_value = c_float((check_indices(1)-1)*dim + check_indices(2))
                if (abs(result_value - check_value) > 1.0e-3) then
    ...
    subroutine perform_double(dim, loop_num, ave_gflops, max_gflops, ave_time, min_time, my_rank, num_procs)
        ...
        do i = 1, loop_num
            if (my_rank == 0) then
                ...
                check_value = 0.0d0
                do k = 1, dim
                    check_value = check_value + a_double((check_indices(1)-1)*dim + k) * b_double((k-1)*dim + check_indices(2))
                end do
            end if
            ...
            ! 主进程进行校验
            if (my_rank == 0) then
                result_value = c_double((check_indices(1)-1)*dim + check_indices(2))
                if (abs(result_value - check_value) > 1.0d-6) then
                    print *, 'Verification failed at iteration ', i, ': expected ', check_value, ', got ', result_value
                end if
            end if
