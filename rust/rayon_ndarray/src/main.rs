...
fn execute_float(dim: usize, loop_num: usize) -> (f64, f64, f64, f64) {
    ...
    for i in 0..loop_num {
        ...
        // 生成随机校验点
        let mut rng = rand::rng();
        let test_i = rng.random_range(0..dim);
        let test_j = rng.random_range(0..dim);
        // 计算正确值
        let mut correct_value = 0.0f32;
        for k in 0..dim {
            correct_value += a[test_i * dim + k] * b[k * dim + test_j];
        }
        ...
        // 结果校验
        let computed_value = c[test_i * dim + test_j];
        if (computed_value - correct_value).abs() > 1e-3 {
            println!(
                "Validation failed: Coordinates({}, {}) Calculated value={:.6} Correct value={:.6} Error={:.6}",
                test_i,
                test_j,
                computed_value,
                correct_value,
                computed_value - correct_value
            );
        }
...
fn execute_double(dim: usize, loop_num: usize) -> (f64, f64, f64, f64) {
    ...
    for i in 0..loop_num {
        ...
        // 结果校验
        let computed_value = c[test_i * dim + test_j];
        if (computed_value - correct_value).abs() > 1e-6 {
            println!(
                "Validation failed: Coordinates({}, {}) Calculated value={:.6} Correct value={:.6} Error={:.6}",
                test_i,
                test_j,
...
fn main() {
    let matches = Command::new("rayon_ndarray-rs")
        .version("0.1.0")
        .author("AndrewMoa")
